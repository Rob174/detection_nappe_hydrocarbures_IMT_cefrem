...
if __name__ == '__main__':
    with File(
            r"C:\Users\robin\Documents\projets\detection_nappe_hydrocarbures_IMT_cefrem\data_in\filtered_other_cache_images.hdf5",
            "r") as cache_patch:
        with open(
                r"C:\Users\robin\Documents\projets\detection_nappe_hydrocarbures_IMT_cefrem\data_in\filtered_other_img_infos.json",
                "r") as fp:
            dico_infos = json.load(fp)
        buffer = []
        last_src_image = ""
        num_original_images = len(
            list({v for v in list(map(lambda x: dico_infos[x]["source_img"], dico_infos.keys()))}))
        ...
        for name in dico_infos.keys():
            ...
            source_image = dico_infos[name][
                "source_img"]  # Patch extracted from the same augmented image as the previous one
