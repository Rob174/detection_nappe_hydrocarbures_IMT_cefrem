...
class RGB_Overlay_Patch:
    ...
    def __call__(self,name_img,model,blending_factor=0.5):
        original_img = self.dataset.attr_dataset.images[name_img]
        overlay_true = np.zeros((*original_img.shape[:-1],3))
        overlay_pred = np.zeros((*original_img.shape[:-1], 3))
        ...
        for id,[input,output] in enumerate(self.dataset.attr_dataset.all_patches_of_image(name_img)):
            ...
            color_true = np.ones((self.dataset.attr_patch_creator.attr_grid_size_px,
                                  self.dataset.attr_patch_creator.attr_grid_size_px,3))
            color_pred = np.ones((self.dataset.attr_patch_creator.attr_grid_size_px,
                                  self.dataset.attr_patch_creator.attr_grid_size_px,3))
            for i,c in enumerate(output):
                color_true[:,:,i] *= c
            for i,c in enumerate(prediction):
                color_pred[:,:,i] *= c
            overlay_true[pos_x:pos_x+self.dataset.attr_patch_creator.attr_grid_size_px,
                    pos_y:pos_y+self.dataset.attr_patch_creator.attr_grid_size_px,
                    :] = input * (1-blending_factor) + color_true * blending_factor
            overlay_pred[pos_x:pos_x+self.dataset.attr_patch_creator.attr_grid_size_px,
                    pos_y:pos_y+self.dataset.attr_patch_creator.attr_grid_size_px,
