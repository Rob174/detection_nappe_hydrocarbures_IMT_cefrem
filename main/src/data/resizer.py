...
if __name__ == "__main__":
    ...
    plt.imshow(Resizer(out_size_w=256)(img),cmap="gray")
    ...
    plt.imshow(Resizer(out_size_w=256,interpolation=cv2.INTER_NEAREST)(img),cmap="gray")
    plt.savefig(folder + f"{dataset.current_name}_resized_nearest.png")
    plt.clf()
    plt.figure()
    plt.title("Resized 256 px image with nearest LANCZOS4 interpolation")
    plt.imshow(Resizer(out_size_w=256,interpolation=cv2.INTER_LANCZOS4)(img),cmap="gray")
