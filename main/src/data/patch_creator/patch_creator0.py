...
class Patch_creator0(BaseClass):
    ...
    def call(self, image: np.ndarray,image_name: str, patch_id: int,count_reso=False) -> Tuple[np.ndarray,bool]:
        ...
        if Patch_creator0.check_reject(patch,self.attr_exclusion_policy_threshold) is True : # 0 ns or 1 ms (sometimes) for the condition. 1 ms and 5 ms for the len(patch.... . 0 ns for the int(.....
